...
struct vring_used {
        u16 flags;
        u16 idx;
        struct vring_used_elem ring[1025];
};
...
int kprobe__virtqueue_add_outbuf(struct pt_regs *ctx, struct virtqueue *_vq) {
    struct data_t data = {};
    struct vring_virtqueue *vq = (struct vring_virtqueue *)_vq;
    
    int a = vq->vring.used->idx;
    int b = vq->last_used_idx; 
    int more = a - b;
    if (more < 0) 
        more = a + (65536 - b);
    distsd.increment(bpf_log2l(more));

    u32 mfree = vq->vq.num_free;
    u32 key = 0;
...
int kprobe__virtqueue_kick(struct pt_regs *ctx) {
    u32 key = 0;
...
int kprobe__virtqueue_add_inbuf(struct pt_regs *ctx, struct virtqueue *_vq) {
    struct data_t data = {};
    struct vring_virtqueue *vq = (struct vring_virtqueue *)_vq;
    int a = vq->vring.used->idx;
    int b = vq->last_used_idx;
